...
namespace Devon4Net.WebAPI.Implementation.Domain.RepositoryInterfaces
{
    ...
    public interface I${variables.entityName?cap_first}Repository : IRepository<${variables.entityName?cap_first}>
    {
        ...
        /// </summary>
        /// <returns>"${variables.entityName?cap_first}"</returns>
        Task<${variables.entityName?cap_first}> Set${variables.entityName?cap_first}(<#list model.properties as property><#if property.isCollection><#if property.type == "number">long<#elseif property.type == "integer">int<#else>List<${property.type}</#if>> ${property.name?uncap_first}<#if property?has_next>, </#if><#else><#if property.type == "number">long<#elseif property.type == "integer">int<#else>${property.type}</#if> ${property.name?uncap_first}<#if property?has_next>, </#if></#if></#list>);
