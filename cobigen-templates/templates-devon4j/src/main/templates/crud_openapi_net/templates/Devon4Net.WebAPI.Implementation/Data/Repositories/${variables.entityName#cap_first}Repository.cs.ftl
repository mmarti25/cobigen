...
namespace Devon4Net.WebAPI.Implementation.Data.Repositories
{
    ...
    public class ${variables.entityName?cap_first}Repository : Repository<${variables.entityName?cap_first}>, I${variables.entityName?cap_first}Repository
    {
        ...
        /// </summary>
        /// <returns>"${variables.entityName?cap_first}"</returns>
        public async Task<${variables.entityName?cap_first}> Set${variables.entityName?cap_first}(<#list model.properties as property><#if property.isCollection><#if property.type == "number">long<#elseif property.type == "integer">int<#else>List<${property.type}</#if>> ${property.name?uncap_first}<#if property?has_next>, </#if><#else><#if property.type == "number">long<#elseif property.type == "integer">int<#else>${property.type}</#if> ${property.name?uncap_first}<#if property?has_next>, </#if></#if></#list>)
        {
